...
class ChannelVM : ViewModel() {
    ...
    //加入RTC频道
    fun joinChannelSDK() {
            RtcManager.instance.joinChannel(getToken(),getChannelId(), getSelfId(), if (isMeHost()) {
                Role.HOST
            } else {
                Role.AUDIENCE
            })
    ...
    fun leaveChannelSDK() {
        ...
        ServiceManager.instance.leaveChannel(if (isMeHost()){1}else{0},channelInfo.value?.channelId.toString())
    }
    ...
    fun getMemberWhenEnterFromHttp() {
        launch({
            ...
            if (isMeHost()){
                var raisedRep = ServiceManager.instance.getRaisedHandsList(getChannelId(),it).await()
                if (raisedRep.code==0){
                    raisedRep.data.forEach {
                        addRaisedHandsMember(RaisedHandsMember(it.uid,it.userName,it.avatar,it.state != 1))
                    }
                }
        ...
        override fun onMessageReceived(var1: RtmMessage?, var2: String?) {
            ...
            when (action) {
                BroadcastCMD.RAISE_HANDS -> {
                    val userName = json.getString("userName")
                    val userIcon = json.getInt("userIcon")
                    addRaisedHandsMember(RaisedHandsMember(var2.toString(),userName,userIcon,false))
                }
        ...
        override fun onUserOffline(uid: String?, reason: Int) {
            ...
            if (reason == 2) {
                launch({
                    val userRep =ServiceManager.instance.getUserInfo(uid.toString(),it).await()
                    if (userRep.code ==0){
                        addListener(createListener(uid.toString(),userRep.data.userName,userRep.data.avatar))
                    }else{
    ...
    private fun createListener(userId: String=getSelfId(),userName:String=getSelf()?.userName!!,userIcon:Int=getSelf()?.userIcon!!):Listener{
